...
{
    [Fact]
    public void Model_contains_six_business_tables_with_required_relationships()
    {
        ...
        db.Model.FindEntityType(typeof(WaterConnectionRequest))!.FindPrimaryKey().Should().NotBeNull();
        ...
        db.Model.FindEntityType(typeof(ServiceRequest))!.GetForeignKeys()
            .Should().Contain(foreignKey => foreignKey.PrincipalEntityType.ClrType == typeof(Resident));
        db.Model.FindEntityType(typeof(WaterConnectionRequest))!.GetForeignKeys()
            .Should().Contain(foreignKey => foreignKey.PrincipalEntityType.ClrType == typeof(Resident));
    }
}
